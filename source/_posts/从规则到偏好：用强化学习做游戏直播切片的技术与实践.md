...
categories:
  - AI应用
  - 视频处理
description: "## 从规则到偏好——用强化学习做游戏直播AI切片

### 1. 引言

本文记录了一套游戏直播自动切片系统的完整设计与实战过程。系统以「三角洲行动」FPS 直播为场景，通过语音转写 + 画面OCR + 弹幕联合分析自动识别高光时刻，并引入基于成对偏好学习的强化学习模型替代传统硬编码规则，使切片策略随用户标注逐步优化。

传统直播切片依赖人工逐帧回放筛选，一段 1 小时的录播通常需要 2-3 小时剪辑。本文展示的系统将这一流程压缩到约 10 分钟（含 GPU 转写 + OCR + 分析），并让模型从用户反馈中持续学习偏好。

### 2. 技术栈

- **语音转写**: faster-whisper large-v3 (CUDA float16)
- **画面OCR**: PaddleOCR GPU + ffmpeg NVDEC 硬件解码
- **RL模型**: LogisticRegression 成对偏好学习 (scikit-learn)
- **视频切割**: ffmpeg NVENC GPU 硬件编码
- **弹幕分析**: 密度曲线 + 关键词分类 + 高频词提取
- **特征工程**: 35 维多模态特征向量

### 3. 核心成果

- 51 分钟直播自动识别 25 个候选切片，覆盖率 43.2%
- RL 模型训练 11 次后，从 9 个成对比较学习到用户偏好（纯击杀 > 弹幕高潮 > 搞笑）
- 13 个最终切片全部采用 GPU 硬件切割，总耗时 < 2 分钟
- 弹幕关键词权重实证：好评词 -0.16、笑声词 -0.27（与用户选择一致）
- 模型可解释：每个特征权重直接对应特征贡献方向"
---
